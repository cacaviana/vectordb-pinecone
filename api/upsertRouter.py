...
async def upsert_vectorstore(
    ...
    try:
        ...
        upsert_response = upsert_document(
        ...
        return JSONResponse(
            content={"error": f"Erro ao processar upsert: {str(e)}"}, 
            status_code=500
        )
...
async def upsert_vectorstore_metadados(
    filepdf: UploadFile, 
    index_name: str = Form(...), 
    namespace: str = Form(...), 
    metadone: str = Form(...)
    ...
    try:
        ...
        return JSONResponse(
            content={"error": f"Erro ao processar metadados JSON: {str(e)}"}, 
            status_code=400
        )
...
async def upsert_vectorstore_clientfinance(
    filepdf: UploadFile, 
    index_name: str = Form("client-finance"), 
    namespace: str = Form("ClientFinance")
):
    
    try:
        TextFromPDF = extract_text_from_pdf(filepdf)
        chucklist = split_text_into_chunks_maiores(text=TextFromPDF)
        upsert_response = upsert_document_semmetadados(
            index_name=index_name, 
            chunks=chucklist, 
            namespace=namespace
        )
